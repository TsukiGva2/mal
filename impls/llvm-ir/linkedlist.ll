...
; TODO: allow multiple linked lists at once
; by using indexes (like a list stack)
...
define void @LL.Attach(%LL* %node) {

  ; attach a linked list to the current
  ; tail.

  %isNull = icmp eq %LL* %node, null
  br i1 %isNull, label %no.op, label %attach
...
attach:
  ...
  ; get a pointer to the Next field from the tail
  %Tail.Next = getelementptr %LL, %LL* %Tail, i32 0, i32 1

  ; store the attached node as the next
  store %LL* %node, %LL** %Tail.Next
  
  br label %findTail

findTail:
  ; find out our new last node

  %current = alloca %LL*
  store %LL* %node, %LL** %current

  br label %iter

iter:
  %ldCurrent = load %LL*, %LL** %current

  %next = call %LL*
    @LL.GetNext(%LL* %ldCurrent)

  br label %check

check:
  %atTail = icmp eq %LL* %next, null
  br i1 %atTail, label %done, label %advance

advance:
  store %LL* %next, %LL** %current
  br label %iter

done:
  store %LL* %ldCurrent, %LL** @LL.Tail
